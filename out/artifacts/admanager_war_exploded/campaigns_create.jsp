...
<div class="container-fluid">
    ...
    <form class="form-horizontal" action="#" id="formAdmob">
        ...
        <div class="form-group">
            <label for="selectLanguage" class="col-sm-2 control-label">语言</label>
            <div class="col-sm-10">
                <select class="form-control select2" id="selectLanguageAdmob" multiple="multiple">
                </select>
            </div>
        </div>
        <div class="form-group">
...
<script>
    ...
    var languageList = ["null", "English", "Arabic", "Spanish", "Portuguese", "French", "Russian"];
    ...
    function init() {
        ...
        $('#btnCreateAdmob').click(function() {
            ...
            var accountId = $('#selectAccountAdmob').val();
...
//            标签名_地理位置&性别&年龄&设备&操作系统_语言_账号_广告图路径
            ...
            $('#inputCampaignName').val(appName + "_" + region.join(",") + gendar + age + userDevice + userOs + "_" + language + "_" + accountName + "_" + imagePath);
        });
...
//            标签名_地理位置&语言&出价_创建时间
   ...
   $('#selectApp,#selectLanguage').change(function () {
        var appName = $('#selectApp').val();
        var language = $('#selectLanguage').val();
        var checkFacebook = $('#checkFacebook:checked').val()=="on"?"true":"false";

        var checkAdmob = $('#checkAdmob:checked').val()=="on"?"true":"false";

        $.post("campaign/selectMessage", {
            appName: appName,
            language: language,
            checkFacebook : checkFacebook,
            checkAdmob : checkAdmob
        }, function (data) {
            appList = data.array;
            if(checkFacebook){
                appList.forEach(function(one) {
                    titleList.push(one.title);
                    messageList.push(one.message);
                });
                $('#inputTitle').autocomplete({source: titleList});
                $('#inputMessage').autocomplete({source: messageList});
            }
            if(checkAdmob){
                appList.forEach(function(one) {
                    messageList1.push(one.message1);
                    messageList2.push(one.message2);
                    messageList3.push(one.message3);
                    messageList4.push(one.message4);
                });
                $('#inputMessage1').autocomplete({source: messageList1});
                $('#inputMessage2').autocomplete({source: messageList2});
                $('#inputMessage3').autocomplete({source: messageList3});
                $('#inputMessage4').autocomplete({source: messageList4});
            }
        }, "json");
       return false;
    });
    $('#inputTitle').change(function () {
        var title = $(this).val();
        $.each(titleList,function(index,e) {
            if(title == e){
               $('#inputMessage').val(messageList[index]);
                return false;
            }
        });
    });
