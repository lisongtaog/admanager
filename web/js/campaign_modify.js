...
function init() {
     ...
     * 而initFormData()是需要三个参数 isAutoCreate, modifyNetwork,modifyRecordId1
     */
    $.post('system/fb_app_id_rel/query', {
        ...
        if (data && data.ret == 1) {
            ...
            if (pendingList.length == 0) {
                if (isAutoCreate && modifyNetwork != null && modifyRecordId1 != null) {
                    initFormData();
                }
            }
        } else {
        ...
        $.post('auto_create_campaign/' + modifyNetwork + '/query_by_id', {
            ...
            if (data && data.ret == 1) {
                ...
                if (modifyNetwork == 'facebook') {
                    ...
                    //FB主页
                    var page_ids = campaignData.page_id.split(",");
                    $("#selectFBPage").val(page_ids);
                    $('#selectFBPage').trigger('change');
                    ...
                    //版位
                    $('#selectPublisherPlatforms').val(campaignData.publisher_platforms.split(','));
                    $('#selectPublisherPlatforms').trigger('change');
                    //预算
                    $('#inputBudget').val(campaignData.bugdet);
                    //竞价策略
                    $('#selectBidStrategy').val(campaignData.bid_strategy);
                    //应用出价、竞价上限
                    ...
                    //广告标题和信息
                    var group_id = campaignData.group_id;

                    $("#advertisement tbody tr").each(function () {
                        var a = $(this).children();//获取每一行
                        var arr = a[1].innerText;//取得第二列// 列的值
                        if (group_id == arr) {
                            a[0].checked = true;
                            return;
                        }
                    ...
                    //事件
                    $('#selectIncidentAdmob').val(campaignData.conversion_id);
                    $('#selectIncidentAdmob').trigger('change');
                    //预算
                    ...
                    $("#advertisement_admob tbody tr").each(function () {
                        var a = $(this).children();//获取每一行
                        var arr = a[1].innerText;//取得第二列// 列的值
                        if (group_id == arr) {
                            a[0].checked = true;
...
function getFBPages(tagName) {
    $.ajax({
        type: "post",
        url: 'system/fb_app_id_rel/queryFBPage',
        async: false,
        datatype: "json",
...
$("#selectAppAdmob").change(function () {
    ...
    $.ajax({
        type: "post",
        url: 'advert_conversion_admob/query_advert_conversion_by_app_name',
        async: false,
        datatype: "json",
        data: {appName: appNameAdmob},
...
$("#selectRegion").change(function () {
    ...
    if (region != null && region.length > 0) {
        ...
        if (appName != "") {
            ...
            //         admanager.showCommonDlg("Warning", data.message);
            //     }
            //     $("#checkbox_facebook").prop("checked", true);
...
$("#selectRegionAdmob").change(function () {
    ...
    if (regionAdmob != null && regionAdmob.length > 0) {
        ...
        //         admanager.showCommonDlg("Warning", data.message);
        //     }
        //     $("#checkbox_admob").prop("checked", true);
...
function FacebookFormReading() {
    ...
    var maxCPA = $('#inputMaxCpa').val();//关闭价格
    var PublisherPlatforms = $("#selectPublisherPlatforms").val();//版位
...
$('#btnUpdate').click(function () {
    ...
    if (isAutoCreate && modifyRecordId1 > 0) {
        requestPool.forEach(function (p) {
            // p.push({
            //     key: id,
            //     values: modifyRecordId1
            // });
            p.id = modifyRecordId1;
        });
        ...
        batchRequest(requestPool, function (param, onSuccess, onFail) {
            ...
            layer.tips("自动创建系列更新完毕", "#btnCreate", {tips: 1, time: 3000});
        });
    }
    return false;
});
...
$("#btnUpdateAdmob").click(function () {
    ...
    batchRequest(AutoRequestPool, function (param, onSuccess, onFail) {
        ...
        layer.tips("更新处理完毕", "#btnCreateAdmob", {tips: 1, time: 3000});
    });
    return false;
});
