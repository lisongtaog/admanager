 ...
 **/
function getExplodeParams(params, explodeParam) {    //这里的参数是怎么传递进去的？
...
function generateFacebookCampaignName(params) {
    ...
    if (params.videoPath) {
        ...
        videoPath = videoPath.replace(/.*\//,"视频");
        dims.push(videoPath);
    } else {
        var videoPath = $("#inputVideoPath").val().trim().replace(/,$/,"").replace(/.*\//,"视频");
        dims.push(videoPath);
    }
