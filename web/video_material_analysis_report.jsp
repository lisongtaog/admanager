...
<html>
<head>
    <title>视频素材分析报告(仅Facebook)</title>
...
<script>
    $("li[role='presentation']:eq(21)").addClass("active");
    ...
    $("#btnSearch").click(function(){
        ...
        $.post('video_material_analysis_report/query_video_material_analysis_report_by_tag', {
            tagName: query,
            startTime: startTime,
            endTime: endTime
        },function(data){
    ...
    function setData(data) {
        ...
        <!-- video_path行 -->
        var one = arr[0];
        var videoPathsStr = one['video_paths'];
        var videoPathArr = videoPathsStr.split(",");
        var videoPathArrLength = videoPathArr.length;
        var tr = $('<tr></tr>');
        var th = $("<th class='td_left_border td_right_border'></th>");
        th.text("视频路径");
        tr.append(th);
        for(var m = 0; m < videoPathArrLength; m++){
            var videoPath = videoPathArr[m];
