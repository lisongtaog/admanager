...
public class Query3 extends HttpServlet {
    ...
    private JsonObject fetchOneAppDataSummary(long tagId, String startTime, String endTime, boolean admobCheck,boolean sameTime) throws Exception {
        ...
        if(sameTime){
            sql = "select sum(ch.total_spend) as spend, " +
                    ...
                    "where rt.campaign_id = ch.campaign_id and c.campaign_id = ch.campaign_id " +
                    "and date = '" + endTime + "' " +
                    "and c.status != 'removed' ";    //数据库里"="的查询速度是最快的，相比 BETWEEN startTime AND endTime
        }
        JSObject one = DB.findOneBySql(sql);
    ...
    public static String[] FourteenData(long id, String end, String google_package_id) throws Exception {
        ...
        String[] fourteen_arr = new String[6];
...
//            }
            ...
            String date = r.get("date").getAsString();
            //以下开始拼接用于悬浮显示的字符串
            FourteenList.one_day_spend_for_fourteen_days += spend + "("+ date + ")\n";
            FourteenList.one_day_installed_for_fourteen_days += installed + "("+ date + ")\n";
            FourteenList.one_day_cpa_for_fourteen_days += cpa + "("+ date + ")\n";
            FourteenList.one_day_cvr_for_fourteen_days += cvr + "("+ date + ")\n";
            FourteenList.one_day_revenue_for_fourteen_days += revenue + "("+ date + ")\n";
        }
    ...
    private List<JsonObject> AttrTitleData(long tagId, String endTime, boolean admobCheck) throws Exception {
        ...
        List<JSObject> FourteenDaysData = null;
        List<JsonObject> title = new ArrayList<>();
        String FourteenDaysBefore = DateUtil.addDay(endTime,-13,"yyyy-MM-dd");
        String sql =  "select ifnull(sum(ch.total_spend),0)as spend, " +
                "ifnull(sum(ch.total_installed),0) as installed, ifnull(sum(ch.total_impressions),0) as impressions " +
                ",ifnull(sum(ch.total_click),0) as click from " + webAdCampaignsTable + " c, " + webAdCampaignsHistoryTable + " ch, " +
                "(select distinct campaign_id from " + webAdCampaignTagRelTable + " where tag_id = " + tagId + ") rt " +
                "where rt.campaign_id = ch.campaign_id and c.campaign_id = ch.campaign_id " +
                "and date between '" + FourteenDaysBefore + "'and '" + endTime + "'" +
                " and c.status != 'removed' group by date order by date desc";
        FourteenDaysData = DB.findListBySql(sql);

        if( FourteenDaysData.size() < 14 ){
            int a = 1;
        }
        if(FourteenDaysData.size()== 14){
            //设计一个遍历用于取14天的数据
            for (JSObject one:FourteenDaysData) {
                JsonObject jsonObject = new JsonObject();
            ...
            for(int i=0;i<14;i++){
                ...
                jsonObject.addProperty("one_day_spend", 0);
                jsonObject.addProperty("one_day_installed", 0);
                jsonObject.addProperty("one_day_click", 0);
                jsonObject.addProperty("one_day_cpa",0);
                jsonObject.addProperty("one_day_cvr", 0);
    ...
    //以下是取14天的revenue
    private List<JsonObject> AttrTitleData_revenue(String google_package_id, String endTime)throws Exception {

        List<JsonObject> revenue = new ArrayList<>();
        List<JSObject> FourteenDaysData;
        String FourteenDaysBefore = DateUtil.addDay(endTime,-13,"yyyy-MM-dd");
        String sqlR = "select ifnull(sum(revenue),0) as revenues ,date " +
                "from web_ad_country_analysis_report_history where app_id = '"
                + google_package_id + "' and date between '" + FourteenDaysBefore + "'and'" + endTime + "' group by date order by date desc";
        FourteenDaysData = DB.findListBySql(sqlR);
        if(FourteenDaysData.size()==14){
            for (JSObject one:FourteenDaysData) {
                JsonObject jsonObject = new JsonObject();
                Double revenues=0.0;
                Double tem_revenue = one.get("revenues");
                if(tem_revenue!= null){
                    revenues = tem_revenue;
                }
                SimpleDateFormat date_wfc = new SimpleDateFormat("yyyy-MM-dd");
                String date = date_wfc.format(one.get("date"));
            ...
            for(int i=0;i<14;i++){
                ...
                jsonObject.addProperty("revenue",0);
                jsonObject.addProperty("date",DayCount);
                revenue.add(jsonObject);
            }
        }
        return revenue;
    }
