 ...
 * Desc: 项目组的总收入与总支出
 */
@WebServlet(name = "ProjectTeamTotalRevenueAndTotalCost2", urlPatterns = "/project_team_total_revenue_and_total_cost2/*")
public class ProjectTeamTotalRevenueAndTotalCost2 extends BaseHttpServlet {
        ...
        if (path.matches("/query_project_team_total_revenue_and_total_cost")) {
            ...
            try {
                ...
                for(JSObject team : teamList){
                    ...
                    if (listTag != null && listTag.size() > 0) {
                        for (JSObject t : listTag) {
                            if (t.hasObjectData()) {
                                ...
                                if (x.hasObjectData()) {
                                    totalSpends += NumberUtil.convertDouble(x.get("spend"), 0);
                                }
                                sqlG = "select sum(ch.total_spend) as spend " +
                                ...
                                if (google_package_id != "") {
                                    ...
                                    x = DB.findOneBySql(sqlG);
                                    if (x.hasObjectData()) {
                                        totalRevenues += NumberUtil.convertDouble(x.get("revenues"), 0);
                                    }
                                }
                            }
                        }
                    ...
                    d.addProperty("total_revenues", NumberUtil.trimDouble(totalRevenues, 0));
                    d.addProperty("total_spends", NumberUtil.trimDouble(totalSpends, 0));
                    d.addProperty("total_incomings", NumberUtil.trimDouble(totalIncomings, 0));
                    jsonArray.add(d);
                }
