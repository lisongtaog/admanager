...
public class Query extends HttpServlet {
        ...
        if (isSummary != null) {
            try {
                ...
                if (adwordsCheck != null && adwordsCheck.equals("false") && facebookCheck != null && facebookCheck.equals("false")) {
                    for (int i = 0; i < tags.size(); i++) {
                        ...
                        JsonObject facebook = fetchOneAppDataFacebook(id,tagName,startTime,endTime,0,false,countryCode,likeCampaignName,campaignCreateTime,false,"hasDataCheck");
                        JsonObject admob = fetchOneAppDataAdwords(id,tagName,startTime,endTime,0,false,countryCode,likeCampaignName,campaignCreateTime,false,"hasDataCheck");
                    ...
                    for (int i = 0; i < tags.size(); i++) {
                        ...
                        if("true".equals(facebookCheck)){
                            jsonObject = fetchOneAppDataFacebook(id,tagName,startTime,endTime,0,false,countryCode,likeCampaignName,campaignCreateTime,false,"hasDataCheck");

                        }else if("true".equals(adwordsCheck)){
                            jsonObject = fetchOneAppDataAdwords(id,tagName,startTime,endTime,0,false,countryCode,likeCampaignName,campaignCreateTime,false,"hasDataCheck");
                        }
            ...
            try {
                ...
                if (tagObject.hasObjectData()) {
                    ...
                    String dataCheck = "all";
                    String hasDataCheck = request.getParameter("hasDataCheck");
                    if("true".equals(hasDataCheck)){
                        dataCheck = "hasDataCheck";
                    }
                    String notHasDataCheck = request.getParameter("notHasDataCheck");
                    if("true".equals(notHasDataCheck)){
                        dataCheck = "notHasDataCheck";
                    }
                    if (adwordsCheck != null && adwordsCheck.equals("false") && facebookCheck != null && facebookCheck.equals("false")) {
                        JsonObject admob = fetchOneAppDataFacebook(id, tag,startTime, endTime, sorter, "true".equals(countryCheck), countryCode,likeCampaignName,campaignCreateTime,true,dataCheck);
                        JsonObject facebook = fetchOneAppDataAdwords(id, tag,startTime, endTime, sorter, "true".equals(countryCheck), countryCode,likeCampaignName,campaignCreateTime,true,dataCheck);
                        double total_spend = admob.get("total_spend").getAsDouble() + facebook.get("total_spend").getAsDouble();
                        double total_installed = admob.get("total_installed").getAsDouble() + facebook.get("total_installed").getAsDouble();
                        double total_impressions = admob.get("total_impressions").getAsDouble() + facebook.get("total_impressions").getAsDouble();
                        double total_click = admob.get("total_click").getAsDouble() + facebook.get("total_click").getAsDouble();
                        ...
                        jsonObject = fetchOneAppDataAdwords(id, tag,startTime, endTime, sorter, "true".equals(countryCheck), countryCode,likeCampaignName,campaignCreateTime,true,dataCheck);
                    }else if("true".equals(facebookCheck)){
                        jsonObject = fetchOneAppDataFacebook(id, tag,startTime, endTime, sorter, "true".equals(countryCheck), countryCode,likeCampaignName,campaignCreateTime,true,dataCheck);
                    }
    ...
    private JsonObject fetchOneAppDataFacebook(long tagId, String tagName, String startTime, String endTime,
                                               int sorterId, boolean countryCheck, String countryCode,String likeCampaignName,String campaignCreateTime,boolean hasROI,String dataCheck) throws Exception {
        ...
        if (!campaignIds.isEmpty()) {
            ...
            if("notHasDataCheck".equals(dataCheck)){
                sql = "select campaign_id, account_id, campaign_name, status, create_time, budget, bidding, spend, installed, impressions, click" +
                        ...
                        "and c.status = 'active' and c.campaign_id in (" + campaignIds + ")" +
                        ((countryCode != null && !countryCode.isEmpty()) ? " and country_code='" + countryCode + "'" : "")+
                        "group by ch.campaign_id ) a where impressions = 0 ";
                if (countryCheck) {
                    sql = "select campaign_id, country_code, account_id, campaign_name, status, create_time, budget, bidding, spend, installed, impressions, click" +
                            ", (case when impressions > 0 then click/impressions else 0 end) as ctr" +
                            ", (case when installed > 0 then spend/installed else 0 end) as cpa" +
                            ...
                            ((likeCampaignName != null) ? " and campaign_name like '%" + likeCampaignName +"%' " : "")  +
                            "and date between '" + startTime + "' and '" + endTime + "' " +
                            "and c.campaign_id in (" + campaignIds + ") group by ch.campaign_id ) a where impressions = 0 ";
                }
            }
            sql += orderStr;
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if("hasDataCheck".equals(dataCheck)){
                if (impressions == 0) {
                    continue;
                }
            }
    ...
    private JsonObject fetchOneAppDataAdwords(long tagId, String tagName, String startTime, String endTime,
                                              int sorterId, boolean countryCheck, String countryCode,String likeCampaignName,String campaignCreateTime,boolean hasROI,String dataCheck) throws Exception {
        String relationTable = "web_ad_campaign_tag_admob_rel";
        String webAdCampaignTable = "web_ad_campaigns_admob";
        String webAdCampaignHistoryTable = "web_ad_campaigns_history_admob";
        if (countryCheck || (countryCode != null && !countryCode.isEmpty())) {
        ...
        if("all".equals(dataCheck)){
            if( !campaignIds.isEmpty()){
                sql = "select campaign_id, account_id, campaign_name, create_time, status, budget, bidding, total_spend, total_installed, total_click, total_impressions, cpa,ctr, " +
                        ...
                        " from "+webAdCampaignTable+" where status = 'enabled' and " +
        ...
        }else if("hasDataCheck".equals(dataCheck)){
            if( !campaignIds.isEmpty()){
                sql = "select campaign_id, account_id, campaign_name, status, create_time, budget, bidding, spend, installed, impressions, click" +
                        ", (case when impressions > 0 then click/impressions else 0 end) as ctr" +
                        ", (case when installed > 0 then spend/installed else 0 end) as cpa" +
        ...
        }else if("notHasDataCheck".equals(dataCheck)){
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if(hasROI){
                ...
                double installedI = 0;
                if(countryCode != null && countryCode.length()>0){
                    String sqlC = "select price from web_ad_tag_country_price_dict where tag_name = '" + tagName + "' and country_code = '" + countryCode + "'";
                    JSObject oneI = DB.findOneBySql(sqlC);
                    if(one.hasObjectData()){
                        priceI = Utils.convertDouble(oneI.get("price"),0);
